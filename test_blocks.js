...
if (link === 'https://playentry.org/ws#!/') {
	var color_set = prompt('블럭 색상을 지정해주세요', 'BASIC');
	var outerline_set = prompt('블럭 윤곽선 색상을 지정해주세요', 'BASIC');
} else {
}
...
addBlock('when_ue_keyboard', '%1키보드를 사용했을 때', {
    ...
    } else if (color_set.startsWith('#')) {
	color: String(color_set)
    };
    if (outerline_set.toUpperCase() === 'BASIC') {
        outerline: EntryStatic.colorSet.block.darken.START
    } else if (color_set.startsWith('#')) {
	outerline: String(color_set)
    };
}, {
    params: [
        {
